# CEX N06 -- CEX-N06-COMMERCIAL (CODEX)
...
Write-Host ""
Write-Host "  [*] N06 COMMERCIAL via CODEX" -ForegroundColor Red
...
if (-not $?) {
    git commit -m "[N06] SHOWOFF via codex"
...
try {
    python -c "import sys; sys.path.insert(0, '_tools'); from signal_writer import write_signal; write_signal('n06', 'complete', 9.0, mission='$mission')"
    Write-Host "  [OK] Signal emitted for n06" -ForegroundColor Green
} catch {
