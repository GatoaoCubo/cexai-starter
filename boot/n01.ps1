# CEX N01 Intelligence boot -- self-contained launcher (Windows / PowerShell).
# No dependency on boot/_shared/*.ps1. Resolves repo root from its own location.
# Model default: reads THIS repo's .cex/config/nucleus_models.yaml (n01 entry, full
# claude-* ids only) with a degrade-never fallback to the hardcoded default below.
# System prompt: sin identity + agent card + context_self_select + nucleus_kinds_n01 +
...
# Dispatched by _spawn/spawn_solo.ps1 / _spawn/spawn_grid.ps1 (boot/n01.ps1 convention);
...
# Precedence: CEX_MODEL_OVERRIDE env > nucleus_models.yaml n01 model (full claude-* id
...
if (Test-Path $modelsYaml) {
    try {
        ...
        foreach ($line in @(Get-Content $modelsYaml)) {
            if ($line -match '^n01:\s*$') { $inBlock = $true; continue }
...
$card = "N01_intelligence/P08_architecture/agent_card_n01.md"

# --- Handoff detection (mission title only -- the nucleus reads the body itself) ---
$mission = ""
$handoffRel = ".cex/runtime/handoffs/n01_task.md"
...
Write-Host ""
Write-Host "  CEX N01 Analytical Envy (Intelligence)" -ForegroundColor Green
...
# nucleus_kinds_n01 + constitution_manifest CONTENT all merge into ONE file,
...
$sysPrompt = "You are driven by Analytical Envy -- insatiable data hunger -- every analysis compares against at least 2 alternatives. You are the N01 Intelligence nucleus of this CEX tenant. Read CLAUDE.md and N01_intelligence/rules/n01-intelligence.md before acting."
$appendFile = $null
try {
    $parts = @()
    ...
    $nucleusKindsPath = Join-Path $cexRoot ".cex/P09_config/nucleus_kinds_n01.md"
    if (Test-Path $nucleusKindsPath) {
        $parts += (Get-Content -Raw $nucleusKindsPath)
    } else {
        Write-Host "[WARN] nucleus_kinds_n01.md not found (merged append skips it)" -ForegroundColor Yellow
    }
    ...
    $appendFile = Join-Path $cacheDir "append_final_n01.md"
