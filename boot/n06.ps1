# CEX N06 Commercial boot -- self-contained launcher (Windows / PowerShell).
# No dependency on boot/_shared/*.ps1. Resolves repo root from its own location.
# Model default: reads THIS repo's .cex/config/nucleus_models.yaml (n06 entry, full
# claude-* ids only) with a degrade-never fallback to the hardcoded default below.
# System prompt: sin identity + agent card + context_self_select + nucleus_kinds_n06 +
...
# Dispatched by _spawn/spawn_solo.ps1 / _spawn/spawn_grid.ps1 (boot/n06.ps1 convention);
...
# Precedence: CEX_MODEL_OVERRIDE env > nucleus_models.yaml n06 model (full claude-* id
...
if (Test-Path $modelsYaml) {
    try {
        ...
        foreach ($line in @(Get-Content $modelsYaml)) {
            if ($line -match '^n06:\s*$') { $inBlock = $true; continue }
...
$card = "N06_commercial/P08_architecture/agent_card_n06.md"

# --- Handoff detection (mission title only -- the nucleus reads the body itself) ---
$mission = ""
$handoffRel = ".cex/runtime/handoffs/n06_task.md"
...
Write-Host ""
Write-Host "  CEX N06 Strategic Greed (Commercial)" -ForegroundColor Yellow
...
# nucleus_kinds_n06 + constitution_manifest CONTENT all merge into ONE file,
...
$sysPrompt = "You are driven by Strategic Greed -- maximizes every revenue stream -- never prices without market research. You are the N06 Commercial nucleus of this CEX tenant. Read CLAUDE.md and N06_commercial/rules/n06-commercial.md before acting."
$appendFile = $null
try {
    $parts = @()
    ...
    $nucleusKindsPath = Join-Path $cexRoot ".cex/P09_config/nucleus_kinds_n06.md"
    if (Test-Path $nucleusKindsPath) {
        $parts += (Get-Content -Raw $nucleusKindsPath)
    } else {
        Write-Host "[WARN] nucleus_kinds_n06.md not found (merged append skips it)" -ForegroundColor Yellow
    }
    ...
    $appendFile = Join-Path $cacheDir "append_final_n06.md"
