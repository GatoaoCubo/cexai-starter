# CEX N04 Knowledge boot -- self-contained launcher (Windows / PowerShell).
# No dependency on boot/_shared/*.ps1. Resolves repo root from its own location.
# Model default: reads THIS repo's .cex/config/nucleus_models.yaml (n04 entry, full
# claude-* ids only) with a degrade-never fallback to the hardcoded default below.
# System prompt: sin identity + agent card + context_self_select + nucleus_kinds_n04 +
...
# Dispatched by _spawn/spawn_solo.ps1 / _spawn/spawn_grid.ps1 (boot/n04.ps1 convention);
...
# Precedence: CEX_MODEL_OVERRIDE env > nucleus_models.yaml n04 model (full claude-* id
...
if (Test-Path $modelsYaml) {
    try {
        ...
        foreach ($line in @(Get-Content $modelsYaml)) {
            if ($line -match '^n04:\s*$') { $inBlock = $true; continue }
...
$card = "N04_knowledge/P08_architecture/agent_card_n04.md"

# --- Handoff detection (mission title only -- the nucleus reads the body itself) ---
$mission = ""
$handoffRel = ".cex/runtime/handoffs/n04_task.md"
...
Write-Host ""
Write-Host "  CEX N04 Knowledge Gluttony (Knowledge)" -ForegroundColor Blue
...
# nucleus_kinds_n04 + constitution_manifest CONTENT all merge into ONE file,
...
$sysPrompt = "You are driven by Knowledge Gluttony -- insatiable for completeness -- every fact sourced and cross-referenced. You are the N04 Knowledge nucleus of this CEX tenant. Read CLAUDE.md and N04_knowledge/rules/n04-knowledge.md before acting."
$appendFile = $null
try {
    $parts = @()
    ...
    $nucleusKindsPath = Join-Path $cexRoot ".cex/P09_config/nucleus_kinds_n04.md"
    if (Test-Path $nucleusKindsPath) {
        $parts += (Get-Content -Raw $nucleusKindsPath)
    } else {
        Write-Host "[WARN] nucleus_kinds_n04.md not found (merged append skips it)" -ForegroundColor Yellow
    }
    ...
    $appendFile = Join-Path $cacheDir "append_final_n04.md"
