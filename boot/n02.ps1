# CEX N02 Marketing boot -- self-contained launcher (Windows / PowerShell).
# No dependency on boot/_shared/*.ps1. Resolves repo root from its own location.
# Model default: reads THIS repo's .cex/config/nucleus_models.yaml (n02 entry, full
# claude-* ids only) with a degrade-never fallback to the hardcoded default below.
# System prompt: sin identity + agent card + context_self_select + nucleus_kinds_n02 +
...
# Dispatched by _spawn/spawn_solo.ps1 / _spawn/spawn_grid.ps1 (boot/n02.ps1 convention);
...
# Precedence: CEX_MODEL_OVERRIDE env > nucleus_models.yaml n02 model (full claude-* id
...
if (Test-Path $modelsYaml) {
    try {
        ...
        foreach ($line in @(Get-Content $modelsYaml)) {
            if ($line -match '^n02:\s*$') { $inBlock = $true; continue }
...
$card = "N02_marketing/P08_architecture/agent_card_n02.md"

# --- Handoff detection (mission title only -- the nucleus reads the body itself) ---
$mission = ""
$handoffRel = ".cex/runtime/handoffs/n02_task.md"
...
Write-Host ""
Write-Host "  CEX N02 Creative Lust (Marketing)" -ForegroundColor Magenta
...
# nucleus_kinds_n02 + constitution_manifest CONTENT all merge into ONE file,
...
$sysPrompt = "You are driven by Creative Lust -- seductive, irresistible prose -- brand voice comes first, always. You are the N02 Marketing nucleus of this CEX tenant. Read CLAUDE.md and N02_marketing/rules/n02-marketing.md before acting."
$appendFile = $null
try {
    $parts = @()
    ...
    $nucleusKindsPath = Join-Path $cexRoot ".cex/P09_config/nucleus_kinds_n02.md"
    if (Test-Path $nucleusKindsPath) {
        $parts += (Get-Content -Raw $nucleusKindsPath)
    } else {
        Write-Host "[WARN] nucleus_kinds_n02.md not found (merged append skips it)" -ForegroundColor Yellow
    }
    ...
    $appendFile = Join-Path $cacheDir "append_final_n02.md"
