# CEX N02 LiteLLM -- routes via proxy on :4000
# Mirrors boot/n02_ollama.ps1 but talks to the LiteLLM proxy instead of
...
Write-Host "  [>>] Launching litellm_nucleus.py with model alias cex-n02" -ForegroundColor Green
Write-Host ""

Set-CexTitle "RUNNING"
$cex_start_time = Get-Date
& python "$cexRoot\_tools\litellm_nucleus.py" --nucleus N02 --base-url $proxyUrl
Set-CexTitle "DONE"
