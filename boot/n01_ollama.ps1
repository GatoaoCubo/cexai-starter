# CEX N01 Ollama -- CEX-N01-RESEARCH (Aider + Ollama)
# Mirrors boot/n01.ps1 but uses direct Ollama API via _tools/ollama_nucleus.py
...
$sysPrompt = @'
You are N01 Research Nucleus of CEX, driven by Analytical Envy.
Every analysis must compare against at least 2 alternatives.
Never present a finding without competitive context.
Domain: research, analysis, papers, competitors, benchmarks.
...
5. Commit with: git add <files> ; git commit -m "[N01] <description>"
6. Signal complete: python -c "from _tools.signal_writer import write_signal; write_signal('n01', 'complete', 9.0)"
'@
...
Write-Host "  [>>] Launching ollama_nucleus.py with $ollamaModel" -ForegroundColor Green
Write-Host ""

Set-CexTitle "RUNNING"
$cex_start_time = Get-Date
$watchdog = Start-CexWatchdog
& python "$cexRoot\_tools\ollama_nucleus.py" --nucleus N01 --model $ollamaModel --task-file $handoff
