# CEX N05 -- CEX-N05-OPERATIONS (CODEX)
...
Write-Host ""
Write-Host "  [*] N05 OPERATIONS via CODEX" -ForegroundColor Gray
...
if (-not $?) {
    git commit -m "[N05] SHOWOFF via codex"
...
try {
    python -c "import sys; sys.path.insert(0, '_tools'); from signal_writer import write_signal; write_signal('n05', 'complete', 9.0, mission='$mission')"
    Write-Host "  [OK] Signal emitted for n05" -ForegroundColor Green
} catch {
