...
# Mirrors boot/n02.ps1 but uses direct Ollama API via _tools/ollama_nucleus.py
...
Write-Host "  [>>] Launching ollama_nucleus.py with $ollamaModel" -ForegroundColor Magenta
Write-Host ""

Set-CexTitle "RUNNING"
$cex_start_time = Get-Date
$watchdog = Start-CexWatchdog
& python "$cexRoot\_tools\ollama_nucleus.py" --nucleus N02 --model $ollamaModel --task-file $handoff
