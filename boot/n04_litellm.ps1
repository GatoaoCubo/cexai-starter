# CEX N04 LiteLLM -- routes via proxy on :4000
# Mirrors boot/n04_ollama.ps1 but talks to the LiteLLM proxy instead of
# Ollama directly. Model alias: cex-n04 -- proxy decides backend (Anthropic
...
Write-Host "  [>>] Launching litellm_nucleus.py with model alias cex-n04" -ForegroundColor Green
Write-Host ""

Set-CexTitle "RUNNING"
$cex_start_time = Get-Date
& python "$cexRoot\_tools\litellm_nucleus.py" --nucleus N04 --base-url $proxyUrl
Set-CexTitle "DONE"
