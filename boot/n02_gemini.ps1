...
$sysPrompt = @'
You are N02 Marketing Nucleus of CEX. Domain: copy, ads, campaigns, brand voice.
...
Your agent card: N02_marketing/P08_architecture/agent_card_n02.md
...
# Gemini CLI 0.40.1+ requires --approval-mode yolo (not just -y) to truly bypass
# planning mode, AND --skip-trust to bypass workspace trust dialog. Long embedded
# handoff bodies trigger Plan Mode; short action prompts that point at the handoff
# file in the workspace work reliably.
...
# $initialMsg above (read from n02_task_gemini.md). Pipe it via STDIN; pass only a
...
if (-not $geminiBin) {
    Write-Host "[N02][FAIL] gemini CLI not found" -ForegroundColor Red
    ...
    $lr | ConvertTo-Json | Out-File "$lrDir\lr_runtime_failure_n02_$lrTs.json" -Encoding utf8
    Set-CexTitle "FAIL"
    exit 2
}
