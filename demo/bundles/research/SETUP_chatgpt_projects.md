...
Setup do bundle CEXAI `research` (Research) no ChatGPT. Este bundle não usa
Actions, tools externas nem chaves de API -- é um agente 100% de geração de
texto (brief de inteligência/pesquisa), então QUALQUER plano ChatGPT funciona.
~5 minutos.
...
O agente deve produzir um artefato `knowledge_card` estruturado, com fontes
e um veredito de confiança, na voz de marca configurada.
...
- **O agente inventa dado, preço ou fonte** -> reforce nas
