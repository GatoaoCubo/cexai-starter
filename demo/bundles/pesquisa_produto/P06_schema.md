...
purpose: Schema formal para o knowledge_card -- FONTE ÚNICA DA VERDADE
...
8f: "F1_constrain"
keywords:
  - "schema formal para knowledge_card"
...
| author | string (nunca o orchestrator) | SIM | -- | H06, H10 |
...
| when_to_use | string (contexto específico, nunca "quando necessário") | SIM | -- | H06 |
| axioms | list[string], tamanho >= 1, forma ALWAYS/NEVER/IF-THEN | SIM | -- | S18 |
## Campos de Frontmatter (Extensão CEX -- 5)
...
| data_source | URL ou referência de artifact | REC | S15 |
## Padrão de ID
Regex: `^p01_kc_[a-z][a-z0-9_]+$`
Regra: o id DEVE ser igual ao nome do arquivo sem extensão (H02). Somente underscores.
## Convenções de Nomenclatura (populações de nome de arquivo estruturais)
O Regex acima é o gate de checagem para ids novos -- ele não rege retroativamente 6 populações
comprovadas ESTRUTURAIS (LOAD-BEARING) ou INADMISSÍVEIS AO PADRÃO por quebra real durante a
varredura de renomeação R-307 lane-4 (af9552aaaf reverteu 191/383 renomeações exatamente por
isso). O `identity_doctor` (`_tools/cex_check_registry.py`) carrega as MESMAS 6 na sua constante
`EXEMPT_ID_CONVENTIONS` (R-314): removidas do total de mismatch contado, mas sempre visíveis via
`exempted_by_convention`.
| População | Contagem | Exemplo | Por que é exceção |
|---|---|---|---|
| `library/kind/kc_{kind}.md` (simples) | 156 | `kc_ab_test_config` | `load_kc_library()` faz glob de `kc_*.md` por nome de arquivo; ~14 módulos de _tools constroem esse caminho a partir do nome do kind |
...
| `kc_competitor_hermes` | 1 | (nome exato) | `cex_hygiene.py` R09 fixa esse nome de arquivo no código |
| `kc_8f_*` (começa com dígito) | 5 | `kc_8f_mode_a` | INADMISSÍVEL AO PADRÃO: H02 exige `[a-z]` depois do prefixo; "8f" começa com dígito -- nenhuma renomeação pode admitir isso |
Nenhuma das 6 é "corrigida" retroativamente aqui -- espelha o precedente do próprio
`bld_schema_output_template.md`, "resolvido mas não abençoado" (R-299). Novos ids de
knowledge_card ainda devem ser criados seguindo o Regex acima; esta é uma lista de exceção
fechada e citada, não um precedente para nova divergência.
...
```
Ambas as chaves `primary` e `related` são obrigatórias (S20).
...
7. `## Referências` -- referências de artifact + URLs
...
6. `## Referências` -- referências de artifact + URLs
...
- sem autorreferência no tldr -- S02
