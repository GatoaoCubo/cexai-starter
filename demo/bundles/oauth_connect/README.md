# CEXAI capability bundle: OAuth Connect (`oauth_connect`)

The **12 pillar contract** for the `oauth_app_config` kind, plus the setup config.
Nucleus N03 . kind `oauth_app_config` . pillar P04.
...
becomes a working OAuth Connect agent.
...
generic, public builder contract for `oauth_app_config` -- no tenant data.
