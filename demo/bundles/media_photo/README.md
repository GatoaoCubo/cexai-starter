# CEXAI capability bundle: Media and Photo (`media_photo`)

The **12 pillar contract** for the `multimodal_prompt` kind, plus the setup config.
...
becomes a working Media and Photo agent.
...
generic, public builder contract for `multimodal_prompt` -- no tenant data.
