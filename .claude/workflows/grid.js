export const meta = {
  ...
  whenToUse:
    'Default path for same-runtime (Claude-only), single-session, bounded-duration grid dispatch once handoff files already exist (produced by /guide + /spec, or written directly). Requires args {mission: string}. No per-cell git commit -- N07 consolidates after reviewing the returned results. Companion, not a replacement, for Mode X: cross-runtime cells (gemini/codex/ollama), multi-hour detached/overnight autonomy, and founder-launched OS windows still route through bash _spawn/dispatch.sh grid MISSION [-w] -- see .claude/rules/n07-orchestrator.md and docs/NUCLEUS_ARCHITECTURE_DOSSIER.md (Tier 2, register row R-008).',
  phases: [
