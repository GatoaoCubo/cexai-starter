---
kind: instruction
id: bld_instruction_pitch_deck
pillar: P03
llm_function: REASON
purpose: Step-by-step production process for pitch_deck
quality: null
title: "Instruction Pitch Deck"
version: "1.0.0"
author: wave1_builder_gen_v2
tags: [pitch_deck, builder, instruction]
tldr: "Step-by-step production process for pitch_deck"
domain: "pitch_deck construction"
created: "2026-04-14"
updated: "2026-04-14"
8f: "F6_produce"
keywords: [pitch_deck construction, instruction pitch deck, pitch_deck, builder, instruction, related artifacts, problem solution, slide, problem, solution]
density_score: 0.85
related:
  - pitch-deck-builder
  - p05_qg_pitch_deck
  - bld_knowledge_card_pitch_deck
  - p10_mem_pitch_deck_builder
  - bld_output_template_pitch_deck
---
## Phase 1: RESEARCH  
1. Define target audience (investors, customers) and their pain points.  
2. Analyze competitors’ pitch decks to identify common structures and gaps.  
3. Document the problem statement with data (e.g., market size, customer surveys).  
4. Outline the solution’s unique value proposition and technical feasibility.  
5. Gather traction metrics (users, revenue, partnerships, milestones).  
6. Define the ask (funding amount, use of funds, exit expectations).  

## Phase 2: COMPOSE  
1. Use bld_schema_pitch_deck.md to structure slides: Problem, Solution, Traction, Ask.  
2. Write problem slide: 1 sentence headline, 3 supporting data points.  
3. Write solution slide: 1 core feature, 2 technical differentiators.  
4. Write traction slide: 1 metric headline, 2 visual charts (e.g., growth, adoption).  
5. Write ask slide: 1 funding target, 2 use-of-funds bullet points.  
6. Reference bld_output_template_pitch_deck.md for slide layout, fonts, and color codes.  
7. Add visuals: 1 diagram per problem/solution, 1 customer testimonial.  
8. Insert 1-2 icons per slide for visual hierarchy (e.g., rocket for growth).  
9. Proofread for conciseness (max 10 words per bullet, no jargon).  

## Phase 3: VALIDATE  
- [ ] Peer review: Does each slide align with bld_schema_pitch_deck.md?
- [ ] Data accuracy: Are metrics sourced from verified reports?
- [ ] Consistency: Does design match bld_output_template_pitch_deck.md?
- [ ] Clarity: Can a non-expert explain the problem/solution in 1 minute?
- [ ] Approval: Signed off by CEO and CTO for technical and business claims.

## Related Artifacts
| Artifact | Relationship | Score |
|----------|-------------|-------|
| [[pitch-deck-builder]] | downstream | 0.45 |
| [[p05_qg_pitch_deck]] | downstream | 0.45 |
| [[bld_knowledge_card_pitch_deck]] | upstream | 0.45 |
| [[p10_mem_pitch_deck_builder]] | downstream | 0.43 |
| [[bld_output_template_pitch_deck]] | downstream | 0.37 |
