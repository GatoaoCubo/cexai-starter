-- 20260616000002_tenant_data.sql
-- Multi-tenant data surface: tenant_data (generic business data) + a SINGLE
-- PERMISSIVE tenant-match RLS boundary.
-- Mission MULTITENANT_DATA_PLANE (codename tenant-zero), Wave 2 task T5 -- the
-- first surface to REPLICATE the template-first memory pattern (20260616000001).
...
--   B.1 (tenant_data table + index, copied verbatim), B.2 (PERMISSIVE tenant-match
--   boundary, byte-pattern replica of tenant_memory), C.1 (the cross-tenant-denial
--   gate this file must satisfy symmetrically with the memory surface).
--
...
-- ============================================================================
-- SURFACE: data  (generic tenant business data -- replicates the memory pattern)
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_tenant_data_tenant ON tenant_data(tenant_id);   -- D2 MANDATORY
...
CREATE POLICY tenant_boundary ON tenant_data
  AS PERMISSIVE
...
CREATE POLICY service_role_all ON tenant_data
  AS PERMISSIVE
  FOR ALL
  TO service_role
  USING (true)
