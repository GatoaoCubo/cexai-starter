...
| Skill/plugin registry | Filesystem of named skills and plugins, each a self-contained capability unit | OVERLAP -- CEXAI has the `skill` kind (P03), 119 builders, capability_registry -- identical architectural pattern | SKIP (already covered) | Existing `skill` + `capability_registry` |
